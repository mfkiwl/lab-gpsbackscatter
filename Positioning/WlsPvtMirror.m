function [xHat,z,svPos,H,Wpr,Wrr] = WlsPvtMirror(prs,gpsEph,xo)
...
% 卫星对称映射
sv_length=length(svXyzTtx(:,1));
svXyzTtx_BKS_mirrored=ones(size(svXyzTtx));
for i=1: sv_length
    Temp=mirrorTransform(svXyzTtx(i,:),xo(1:3)');
...
svXyzTrx = svXyzTtx_BKS_mirrored; %initialize svXyz at time of reception
